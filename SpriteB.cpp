...
/// 静的メンバ変数の実体
/// </summary>
ID3D12Device* SpriteB::device = nullptr;
UINT SpriteB::descriptorHandleIncrementSize;
ID3D12GraphicsCommandList* SpriteB::cmdList = nullptr;
ComPtr<ID3D12RootSignature> SpriteB::rootSignature;
ComPtr<ID3D12PipelineState> SpriteB::pipelineState;
XMMATRIX SpriteB::matProjection;
ComPtr<ID3D12DescriptorHeap> SpriteB::descHeap;
ComPtr<ID3D12Resource> SpriteB::texBuff[srvCount];

void SpriteB::StaticInitialize(ID3D12Device* device, int window_width, int window_height)
{
	// nullptrチェック
	assert(device);

	SpriteB::device = device;
...
void SpriteB::LoadTexture(UINT texnumber, const wchar_t* filename)
{
	// nullptrチェック
	assert(device);
...
void SpriteB::PreDraw(ID3D12GraphicsCommandList* cmdList)
{
	// PreDrawとPostDrawがペアで呼ばれていなければエラー
	assert(SpriteB::cmdList == nullptr);

	// コマンドリストをセット
	SpriteB::cmdList = cmdList;
...
void SpriteB::PostDraw()
{
	// コマンドリストを解除
	SpriteB::cmdList = nullptr;
}

SpriteB* SpriteB::Create(UINT texNumber, XMFLOAT2 position, XMFLOAT4 color, XMFLOAT2 anchorpoint, bool isFlipX, bool isFlipY)
{
	// 仮サイズ
	...
	// Spriteのインスタンスを生成
	SpriteB* sprite = new SpriteB(texNumber, position, size, color, anchorpoint, isFlipX, isFlipY);
	if (sprite == nullptr) {
		return nullptr;
	}
...
SpriteB::SpriteB(UINT texNumber, XMFLOAT2 position, XMFLOAT2 size, XMFLOAT4 color, XMFLOAT2 anchorpoint, bool isFlipX, bool isFlipY)
{
...
void SpriteB::SetRotation(float rotation)
{
...
void SpriteB::SetPosition(const XMFLOAT2& position)
{
	this->position = position;
...
void SpriteB::SetSize(const XMFLOAT2& size)
{
	this->size = size;
...
void SpriteB::SetAnchorPoint(const XMFLOAT2& anchorpoint)
{
	this->anchorpoint = anchorpoint;
...
void SpriteB::SetIsFlipX(bool isFlipX)
{
	this->isFlipX = isFlipX;
...
void SpriteB::SetIsFlipY(bool isFlipY)
{
	this->isFlipY = isFlipY;
...
void SpriteB::SetTextureRect(const XMFLOAT2& texBase, const XMFLOAT2& texSize)
{
