...
//静的メンバ変数の実態
const std::string FbxLoader::sBaseDirectory = "Resources/Models/FBX/";
...
void FbxLoader::LoadModelFromFile(FbxModel* model, const string& modelName)
{
    //モデルと同じ名前のフォルダから読み込む
    const string directryPath = sBaseDirectory + modelName + "/";
...
void FbxLoader::ParseNodeRecursive(FbxModel* model, FbxNode* fbxNode, Node* parent)
{
    ...
    node.transform *= matTranslation;   //ワールド行列の回転を反映
...
void FbxLoader::ParseSkin(FbxModel* model, FbxMesh* fbxMesh)
{
    ...
    for (int32_t i = 0; i < clusterCount; i++) {
        ...
        //XMMatrix型に変換する
        XMMATRIX initalPose;
        ConvertMatrixFromFbx(&initalPose, fbxMat);

        //初期姿勢行列の逆行列を得る
        bone.invInitialPose = XMMatrixInverse(nullptr, initalPose);
    }
    ...
    for (int32_t i = 0; i < clusterCount; i++) {
        ...
        for (int32_t j = 0; j < controlPointIndicesCount; j++) {
            ...
            //その頂点の影響を受けるボーンリストに、ボーンえおウェイトのペアを追加
